...
| HTML render                | `src/results/render-html.ts`                                   | Index + per-repo + per-bundle deep-link pages. Explicit unknown no-data state, distinct from failure (CMO C4). stale_since badge. as-of = min(ingested_at). Per-predicate counts ONLY.                                                                                                                                                                                                      |
...
| HTML render                | `src/skills/render-skills.ts`                                       | THREE separate single-dimension renderers (`renderAdoptionPanel` / `renderHumanTrustPanel` / `renderQualityPanel`) — **none takes more than one dimension; there is deliberately no `renderRolledScore`.** Reuses `esc`/`slug`/`SITE_HEADER`/`SITE_FOOTER`/`noDataPanel` from `render-html.ts`. no-data stays visibly labeled (`badge--no-data`, distinct unknown state). No renderer arithmetic (counts printed verbatim, never divided).   |
...
The top-of-landing strip (one row per source repo × 24 hourly buckets, colored by decision mix) and the `/status` USE-method view of the ingest pipeline. Mandatory per DR-035 C4 (Gregg "if absent, the dashboard is useless"). The load-bearing binding: **absence is shown loudly, never silently filled** — an hour with no verified data is an explicit `no-data` unknown outcome, visually distinct from `fail`, and is NEVER carried forward, inferred, blanked, or treated as a pass. Empty hourly buckets are not automatically overdue; publication cadence is not modeled here. Collection status is derived only from recorded worker outcomes, never from test-result silence.
...
| HTML render  | `src/freshness/render-strip.ts`                            | Strip fragment (injected into landing) + the `/status` page. `no-data` has a distinct outlined `bucket--no-data` class; confirmed failures alone use red. Color-blind-safe glyphs + `sr-only` text. No predicate-URI dimension at all → structurally C3-clean.                                                               |
...
**Status-state clarification (2026-09-17):** DR-035 C4 calls for a distinct, visible absence color, not identical failure red. Missing test evidence remains explicit and unknown; only confirmed failures are red. The live single-pass collector does not instrument restart pressure, so it sets `measured: false` and the retries card says **Not recorded**. Instrumented supervisors retain their measured retry counts and escalation alarms. Do not substitute a placeholder zero for unmeasured telemetry.
...
one arrived, read it if so, and otherwise proceed on CI. The deterministic merge gate is this repo's own CI (the C3 no-aggregate-PASS gate + lint:uptime + audit-harness verify + ingest/deploy gates) plus CodeQL. That was
...
repo the reviewer should be pointed at the DR-035 § 8 hard integrity refusals — no cross-predicate aggregate PASS%, no predicate URIs at `labs.*`, no-data-shown-loudly (never blanked or carried forward), and verify-before-render at ingest.
